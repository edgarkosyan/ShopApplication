...
//MARK: WISHLIST FUNCTIONS
        ...
        if User.currentUser() != nil {
            downloadWishListFromFirebase(ownerId: User.currentId()) { (wishList) in
                if wishList == nil {
                    ...
                    wishList?.productsId.append(self.product.id)
                    self.updateWishList(wishList: wishList!, values: ["productId" : wishList!.productsId])
                }
            }
        }else{
            self.hud.textLabel.text = "Please Sign in or Create an Account"
    ...
    func setProduct(){
        ...
        addToWishListButton.layer.cornerRadius = 4
    }
